...
import { isWorkspace, type SpecConfig, type SpeculatorASTSchema } from '@openuji/speculator';
import {
...
async function run(): Promise<void> {
    ...
    if (useMappedMode) {
        if (!args.semanticIrPath || !args.bikeshedConfigPath) {
            throw new Error('Both --semantic-ir and --bikeshed-config are required in mapped mode.');
        }
    ...
    if (args.dryRun) {
        ...
        if (emitted.diagnostics.length > 0) {
            ...
            for (const diagnostic of emitted.diagnostics) {
                ...
                console.log(`[${diagnostic.level}] ${diagnostic.code}: ${diagnostic.message}${path}`);
            }
        }
        return;
    }
    ...
    if (useMappedMode) {
        workspaceOutPath = resolve(args.workspaceOutPath ?? join(outDir, 'workspace.json'));
        await mkdir(resolve(outDir), { recursive: true });
        await writeFile(workspaceOutPath, `${JSON.stringify(workspace, null, 2)}\n`, 'utf-8');
    }
    ...
    if (result.diagnostics.length > 0) {
        ...
        for (const diagnostic of result.diagnostics) {
            const path = diagnostic.path ? ` (${diagnostic.path})` : '';
            console.warn(`  [${diagnostic.level}] ${diagnostic.code}: ${diagnostic.message}${path}`);
        }
    }
}
