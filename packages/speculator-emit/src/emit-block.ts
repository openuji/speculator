...
function serializeHeadingAttrToken(
  key: string,
  value: AttrValue,
): string | null {
...
function emitDefinitionListItem(
  item: DefinitionListItem,
  ctx: EmitContext,
  path: string,
): string {
  const term = wrapHtmlTag(
    "dt",
    {},
    emitInlines(item.term, ctx, `${path}.term`),
  );

  let description: string;
  if (
    item.description.length === 1 &&
    item.description[0]?.type === "paragraph"
  ) {
    const paragraph = item.description[0] as BlockParagraph;
    description = wrapHtmlTag(
      "dd",
      {},
      emitInlines(paragraph.children, ctx, `${path}.description[0].children`),
    );
  } else {
    description = wrapMultilineTag(
      "dd",
      {},
      emitBlocks(item.description, ctx, `${path}.description`),
    );
  }
...
function emitDefinitionList(
  node: BlockDefinitionList,
  ctx: EmitContext,
  path: string,
): string {
  ...
  const inner = node.items
    .map((item, index) =>
      emitDefinitionListItem(item, ctx, `${path}.items[${index}]`),
    )
    .join("\n");
  ...
  if (node.caption.length > 0) {
    parts.push(
      wrapHtmlTag(
        "figcaption",
        {},
        emitInlines(node.caption, ctx, `${path}.caption`),
      ),
    );
  }
    ...
    .map((row, rowIndex) => {
      const cells = row.children
        .map((cell, cellIndex) => {
          ...
          const inner = emitInlines(
            cell.children,
            ctx,
            `${path}.children[${rowIndex}].children[${cellIndex}].children`,
          );
...
function emitSpecStatement(
  ...
  const attrs: Record<string, AttrValue> = {
    ...
    "data-cop-concept":
      "dataCopConcept" in node ? node.dataCopConcept : undefined,
  ...
  const children =
    "children" in node && Array.isArray(node.children)
      ? emitMixedChildren(
          node.children as Array<Block | Inline>,
          ctx,
          `${path}.children`,
        )
      : "";
...
function emitSpecStatementGroup(
  node: Block,
  ctx: EmitContext,
  path: string,
): string {
...
export function emitBlock(node: Block, ctx: EmitContext, path: string): string {
  switch (node.type) {
    ...
    case "html":
      ctx.pushInfo(
        "RAW_HTML_BLOCK",
        "Raw HTML block emitted without transformation.",
        path,
      );
      return node.value;
    case "htmlElement":
