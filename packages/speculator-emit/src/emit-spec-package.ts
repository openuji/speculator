...
function selectDocument(
    ...
    const found = documents.find((document) => document.id === requestedDocumentId);
    if (!found) {
        throw new Error(`Document "${requestedDocumentId}" was not found in workspace.`);
    }
    return found;
}

export function emitSpecPackage(input: EmitSpecPackageInput): EmitSpecPackageResult {
