...
function normalizeHtmlAttributes(
  attrs: HtmlAttributes | undefined,
...
function emitWorkspaceDfnShorthand(
  node: InlineWorkspaceDfnReference,
): string | undefined {
  const canonical = (
    node.targetId?.trim() ||
    node.targetTerm?.trim() ||
    ""
  ).trim();
...
function joinForContexts(
  forContexts: (string | null)[] | undefined,
): string | undefined {
  if (!forContexts || forContexts.length === 0) return undefined;
  const parts = forContexts.filter(
...
function referenceChildren(
  node: WorkspaceReferenceBase | ExternalReferenceBase,
): Inline[] {
...
function sourceToAttrs(
  ...
  if (source.className && source.className.length > 0)
    attrs.class = source.className.join(" ");
...
function emitDefinition(
  ...
  const shouldKeepId = Boolean(
    explicitId && renderedTerm.length > 0 && explicitId !== renderedTerm,
  );

  const attrs: Record<string, AttrValue> = {
    id: shouldKeepId ? explicitId : undefined,
    "data-dfn-type":
      node.dfnType && node.dfnType !== "dfn" ? node.dfnType : undefined,
...
export function emitInline(
  ...
  switch (node.type) {
    ...
    case "workspaceDfnReference": {
      const shorthand =
        options.workspaceDfnShorthand === false
          ? undefined
...
export function emitInlines(
  ...
  return emitInlinesWithOptions(nodes, ctx, path, {
    workspaceDfnShorthand: true,
  });
}
...
function emitInlinesBound(
  ...
  return (nodes, childPath) =>
    emitInlinesWithOptions(nodes, ctx, childPath, options);
}
