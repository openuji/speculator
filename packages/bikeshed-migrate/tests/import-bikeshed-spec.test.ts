    ...
    boilerplateResolver: {
      async resolve(metadata) {
        ...
        const resolved: BoilerplateResult = {
          abstract: {
            content:
              '<section id="abstract"><h2 class="no-num no-toc no-ref" id="abstract">Abstract</h2><p>[ABSTRACT]</p></section>',
            source: "fixture://abstract",
          },
          status: {
            content:
              '<section id="status"><h2>Status of This Document</h2><p>Status boilerplate slot</p><p>[STATUSTEXT]</p></section>',
            source: "fixture://status",
          },
          conformance: {
            content:
              '<div><h2 id="conformance">Conformance</h2><p>Conformance requirements stay around their context. [[!MCP]] and [[!RFC2119]]</p></div>',
          ...
          logo: {
            content:
              '<a class=\"logo\" href=\"https://example.org\"><img src=\"https://example.org/logo.svg\" alt=\"OpenUJI\"></a>',
            source: "fixture://logo",
          },
        };
        return resolved;
...
describe("importBikeshedSpec source extractors", () => {
  ...
  it("extracts style/script resources independently from markdown pipeline", () => {
    const styleBlocks = result.resources.filter(
  ...
  it("integrates boilerplate resolver using metadata-derived group/status", () => {
    ...
    expect(result.boilerplate.conformance?.source).toBe(
      "fixture://conformance",
    );
  });
...
describe("rendered HTML region selection + normalization", () => {
  it("captures selected regions (main, abstract, status)", () => {
    ...
    expect(result.regions.abstract?.selectedHtml ?? "").toMatchSnapshot(
      "selected-abstract",
    );
    expect(result.regions.status?.selectedHtml ?? "").toMatchSnapshot(
      "selected-status",
    );
  });

  it("normalizes selected regions and strips generated chrome", () => {
    expect(result.regions.main.normalizedHtml).toMatchSnapshot(
      "normalized-main",
    );
    expect(result.regions.abstract?.normalizedHtml ?? "").toMatchSnapshot(
      "normalized-abstract",
    );
    expect(result.regions.status?.normalizedHtml ?? "").toMatchSnapshot(
      "normalized-status",
    );
...
describe("semantic importer (HTML -> IR)", () => {
  it("imports main region into a semantic document IR snapshot", () => {
    expect(JSON.stringify(result.document, null, 2)).toMatchSnapshot(
      "document-ir",
    );
  });
  ...
  it("includes required semantic node families", () => {
    ...
    expect(
      linkRefs.some(
        (node) => node.type === "LinkRef" && node.attrs?.dataLinkType === "dfn",
      ),
    ).toBe(true);

    const idlBlock = blocks.find(
      (node): node is IdlBlockNode => node.type === "IdlBlock",
    );
  ...
  it("imports abstract/status region blocks", () => {
    expect(
      JSON.stringify(result.regions.abstract?.blocks ?? [], null, 2),
    ).toMatchSnapshot("abstract-blocks-ir");
    expect(
      JSON.stringify(result.regions.status?.blocks ?? [], null, 2),
    ).toMatchSnapshot("status-blocks-ir");
    expect(
      JSON.stringify(result.regions.conformance?.blocks ?? [], null, 2),
    ).toMatchSnapshot("conformance-blocks-ir");
  });

  it("does not emit empty paragraph nodes", () => {
    expect(
      hasEmptyParagraphNodes(result.document.children as SemanticBlockNode[]),
    ).toBe(false);
    expect(hasEmptyParagraphNodes(result.regions.abstract?.blocks ?? [])).toBe(
      false,
    );
    expect(hasEmptyParagraphNodes(result.regions.status?.blocks ?? [])).toBe(
      false,
    );
    expect(
      hasEmptyParagraphNodes(result.regions.conformance?.blocks ?? []),
    ).toBe(false);
  });

  it("converts boilerplate citation shorthand into semantic biblio references", () => {
    const conformanceText = JSON.stringify(
      result.regions.conformance?.blocks ?? [],
      null,
      2,
    );
    ...
    expect(conformanceText).toContain(
  ...
  it("marks/injects boilerplate sections in semantic IR", () => {
    ...
    const abstractSection = topSections.find(
      (section) => section.boilerplate === "abstract",
    );
    const sotdSection = topSections.find(
      (section) => section.boilerplate === "sotd",
    );
    ...
    expect(JSON.stringify(sotdSection, null, 2)).toContain(
      "Status boilerplate slot",
    );
