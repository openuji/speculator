...
function walkNodes(value: unknown, visitor: (node: Record<string, unknown>) => void): void {
...
function findSectionByBoilerplate(
    ...
    return document.children.find(
        (child): child is Record<string, unknown> =>
            !!child && typeof child === 'object' && (child as { type?: string }).type === 'section' &&
...
describe('mapSemanticIrToSpecAst', () => {
    it('maps webmcp semantic IR and config to speculator AST preserving boilerplate and rich IDL/code semantics', async () => {
        const ir = await readJson<DocumentNode>('../../samples/webmcp/semantic-ir.json');
        const config = await readJson<SpeculatorConfig>('../../samples/webmcp/config.json');
        ...
        walkNodes(document, (node) => {
            if (
                ...
                node.children.some((child) =>
                    typeof child === 'object' &&
                    child !== null &&
                    ['workspaceIdlReference', 'externalIdlReference', 'workspaceDfnReference', 'externalDfnReference'].includes(
            ...
            if (node.type === 'idl' && Array.isArray(node.children) && node.children.length > 0) {
                hasIdlChildren = true;
            }
    ...
    it('maps oidc semantic IR to figure/image asset nodes with mermaid source metadata intact', async () => {
        const ir = await readJson<DocumentNode>('../../samples/oidc/semantic-ir.json');
        const config = await readJson<SpeculatorConfig>('../../samples/oidc/config.json');
    ...
    it('emits diagnostics and falls back to link node for non-inferable external dfn/idl references', () => {
        ...
        const result = mapSemanticIrToSpecAst({ ir, config, sourcePath: '/virtual/test/index.bs' });
