...
function mapDocument(
  ...
  const children = mapDocumentChildren(ir.children, ctx, {
...
function mapSection(
  ...
  const headingChildren = mapInlines(node.heading, ctx, {
    path: `${frame.path}.heading`,
  });
...
function flattenSectionToBlocks(
  node: SectionNode,
  ctx: MapContext,
  frame: MapFrame,
): Block[] {
  ...
  return [
    heading,
    ...mapBlocksInBlockContext(node.children, ctx, {
      path: `${frame.path}.children`,
    }),
  ];
}

function mapBlockNodeToBlocks(
  ...
  switch (node.type) {
    ...
    case "Paragraph": {
      const paragraph: BlockParagraph = {
        type: "paragraph",
        children: mapInlines(node.children, ctx, {
          path: `${frame.path}.children`,
        }),
      };
      return [paragraph];
    ...
    case "List": {
      const list: BlockList = {
        ...
        children: node.items.map(
          (item: ListItemNode, idx: number): ListItem => ({
            type: "listItem",
    ...
    case "FigureBlock": {
      const figure: BlockFigure = {
        type: "figure",
        caption: mapInlines(node.caption, ctx, {
          path: `${frame.path}.caption`,
        }),
        children: mapBlocksInBlockContext(node.children, ctx, {
          path: `${frame.path}.children`,
        }),
  ...
  switch (node.type) {
    ...
    case "CodeSpan": {
      ...
      if (node.children && node.children.length > 0) {
        code.children = mapInlines(node.children, ctx, {
          path: `${frame.path}.children`,
        });
      }
      return code;
...
function mapDefinition(
  ...
  const children = mapInlines(node.children, ctx, {
    path: `${frame.path}.children`,
  });
...
function mapLinkRef(
  ...
  if (node.kind === "dfn" || node.kind === "idl") {
    ...
    return {
      type: "text",
      value: inlinesToPlainText(
        mapInlines(node.children, ctx, { path: `${frame.path}.children` }),
      ),
    };
  }
...
function mapLinkRefToCite(
  ...
  if (!key) {
    pushDiagnostic(ctx, {
      ...
      level: "warning",
      message:
        "Biblio-like LinkRef could not be converted to cite because no key was found.",
      path: frame.path,
    });
    return null;
  }
...
function mapLinkFallback(
  ...
  if (node.kind === "unknown") {
    pushDiagnostic(ctx, {
      ...
      level: "warning",
      message:
        "Unknown reference without href preserved as htmlInlineElement(a).",
      path: frame.path,
    });
  } else {
    pushDiagnostic(ctx, {
...
function createHtmlAnchorInline(
  node: LinkRefNode,
  children: Inline[],
): InlineHtmlElement {
  const attributes: HtmlAttributes = {};
  if (node.attrs?.id) attributes.id = node.attrs.id;
  if (node.attrs?.dataLinkType)
    attributes["data-link-type"] = node.attrs.dataLinkType;
      ...
      switch (node.type) {
        ...
        case "sectionReference":
          return node.children
            ? inlinesToPlainText(node.children)
            : node.targetId;
        case "requirement":
          return node.keyword;
        default:
...
function extractCitationKeyFromHref(
  href: string | undefined,
): string | undefined {
...
function inferXrefSpecFromHref(href: string): string | undefined {
  ...
  try {
    ...
    if (host === "www.rfc-editor.org" || host === "rfc-editor.org") {
      const rfcSegment = pathSegments.find((segment) =>
        /^rfc\d+$/i.test(segment),
      );
    ...
    if (host === "datatracker.ietf.org") {
      const htmlSegment = pathSegments.find((segment) =>
        /^rfc\d+$/i.test(segment),
      );
...
function mapBikeshedConfigToSpecConfig(
  ...
  const id =
    shortName ??
    slugifyId(title ?? inferFallbackIdFromSourcePath(sourcePath) ?? "spec");
...
function mapLocalBiblio(
  input: Record<string, unknown>,
): NonNullable<SpecConfig["localBiblio"]> {
...
function inferFallbackIdFromSourcePath(
  sourcePath: string | undefined,
): string | undefined {
