...
function createSectionFromHeading(
  ...
  const heading = normalizeInlineWhitespace(
    parseInlineChildren(element.children, options),
  );
  const extracted = extractSectionNumber(heading);
  const classList = asClassList(element.properties?.className);
  const noToc =
    classList.includes("no-toc") ||
    hasTruthyBooleanAttr(element, "data-no-toc");
  const noTocCount =
    classList.includes("no-num") ||
    hasTruthyBooleanAttr(element, "data-no-toc-count");
...
function parseBlockElement(
  ...
  if (tag === "p") {
    const children = normalizeInlineWhitespace(
      parseInlineChildren(element.children, options),
    );
...
function parseFigureBlock(
  ...
  for (const child of element.children) {
    ...
    const tag = child.tagName.toLowerCase();
    if (tag === "figcaption") {
      figure.caption = normalizeInlineWhitespace(
        parseInlineChildren(child.children, options),
      );
      continue;
    }
...
function parsePre(
  ...
  if (isIdlPre(element)) {
    return {
      type: "IdlBlock",
      value,
      children: parseInlineChildrenPreserveWhitespace(
        element.children,
        options,
      ),
    };
...
function parseAlgorithmBlock(
  element: Element,
  options: HtmlToIrOptions,
): AlgorithmBlockNode {
  const name =
    getAttr(element, "data-algorithm") ??
    getAttr(element, "dataAlgorithm") ??
    undefined;
...
function parseInlineNode(
  ...
  if (tag === "code") {
    const rawChildren = parseInlineChildrenPreserveWhitespace(
      node.children,
      options,
    );
  ...
  if (tag === "dfn") {
    const definition: DefinitionNode = {
      ...
      children: normalizeInlineWhitespace(
        parseInlineChildren(node.children, options),
      ),
    };
    return [definition];
  }
  ...
  if (tag === "a") {
    const rawChildren = normalizeInlineWhitespace(
      parseInlineChildren(node.children, options),
    );
    const href = getAttr(node, "href");
    const dataLinkType = getAttr(node, "data-link-type");
    const citation = parseBiblioCitationFromChildren(
      rawChildren,
      dataLinkType ?? undefined,
    );
...
function parseInlineNodePreserveWhitespace(
  ...
  if (tag === "a") {
    ...
    const normalizedChildren =
      rawChildren.length > 0
        ? normalizeInlineWhitespace(rawChildren)
        : rawChildren;
    const citation = parseBiblioCitationFromChildren(
      normalizedChildren,
      dataLinkType ?? undefined,
  ...
  if (tag === "code") {
    const children = parseInlineChildrenPreserveWhitespace(
      node.children,
      options,
    );
    const code: CodeSpanNode = {
...
function normalizeInlineWhitespace(
  ...
  return normalized.filter(
    (inline) => inline.type !== "Text" || inline.value.length > 0,
  );
}
...
function isIdlPre(element: Element): boolean {
  const classes = asClassList(element.properties?.className).map((item) =>
    item.toLowerCase(),
  );
  if (classes.includes("idl") || classes.includes("webidl")) return true;

  const nestedCode = findFirstElement(
    element,
    (el) => el.tagName.toLowerCase() === "code",
  );
  if (!nestedCode) return false;

  const nestedClasses = asClassList(nestedCode.properties?.className).map(
...
function inferCodeLanguage(element: Element): string | undefined {
  ...
  const fromData =
    getAttr(element, "data-language") ?? getAttr(element, "dataLanguage");
  ...
  const nestedLanguageClass = asClassList(
    nestedCode.properties?.className,
