...
export async function fetchBoilerplate(
  ...
  const [abstractSlot, statusSlot, copyrightSlot, logoSlot, footerSlot] =
    await Promise.all([
  ...
  const raw = el.children
    .map((child) => {
      ...
      if (child.type === "element")
        return toHtml(child as Element, { closeSelfClosing: true });
      return "";
    })
    .join("");
...
function isPlainP(el: Element): boolean {
  ...
  const hasClass = Array.isArray(p.className)
    ? p.className.length > 0
    : !!p.className;
