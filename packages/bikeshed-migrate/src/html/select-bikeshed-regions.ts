...
export function selectBikeshedRegions(
    ...
    const main =
        findFirstElement(document.root, (el) => el.tagName.toLowerCase() === 'main') ??
        findFirstElement(document.root, (el) => el.tagName.toLowerCase() === 'body');

    if (!main) {
        throw new Error('Could not find <main> (or <body>) in rendered Bikeshed HTML.');
    }
...
function isAbstractRegion(element: Element): boolean {
    ...
    if ((tag === 'section' || tag === 'div') && hasClass(element, 'abstract')) return true;
...
function isStatusRegion(element: Element): boolean {
    ...
    if ((tag === 'section' || tag === 'div') && hasClass(element, 'status')) return true;
