...
export function getAttr(element: Element, name: string): string | undefined {
    ...
    const camelName = name.replace(/-([a-z])/g, (_m, c: string) => c.toUpperCase());
    const raw = props[name as keyof typeof props] ?? props[camelName as keyof typeof props];
...
export function walkElements(root: Root | Element, visitor: (el: Element) => void): void {
