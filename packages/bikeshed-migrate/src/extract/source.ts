 ...
 */
export function extractBikeshedSource(
  content: string,
): ExtractBikeshedSourceResult {
  const { block: metadataBlock, rest: afterMetadata } =
  ...
  const { resources, rest: bodyWithoutResources } =
    extractResources(afterBiblio);
